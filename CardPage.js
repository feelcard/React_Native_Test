           ...
           <View>
                ...
                >
                    ...
                    this.props.dataSet.map((l, i) => (
                        <TouchableOpacity
                        onPress={() => {
                            moveToDetail(JSON.stringify(this.props.dataSet[i].cmpName).replace('"', '').replace('"', ''));
                        }}
                        >
                        <Card style={[styles.card, styles.shadow]} key={i}>
                            ...
                            <View style={{flex:5 , marginLeft: 4}}>
                                <Text style={{fontSize: 20, fontWeight: 'bold'}}>기업명: {JSON.stringify(this.props.dataSet[i]) == undefined ? "새로고침 해주세요" : JSON.stringify(this.props.dataSet[i].cmpName).replace('"', '').replace('"', '') }</Text>
                                <Text style={{fontSize: 16}}>종목코드: {JSON.stringify(this.props.dataSet[i]) == undefined ? "새로고침 해주세요" : JSON.stringify(this.props.dataSet[i].code).replace('"', '').replace('"', '') }</Text>
                            </View>
                        </Card>
                        </TouchableOpacity>
                    ))
